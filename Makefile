...
Kernel/sage/sage.c \
Kernel/sage/command.c \
Kernel/sage/power/power.c
...
run: iso
	qemu-system-i386 -cdrom SageOS.iso -d int

clean:
	rm -f *.o kernel.bin SageOS.iso
	rm -f iso/boot/kernel.bin
	rm -f Kernel/*.o Kernel/sage/*.o Kernel/Drivers/*.o Kernel/Libraires/*.o 2>/dev/null || true
